                          ...
                          PVector ball_pos)
{
    if (side != RIGHT)
    {
        if (ball_pos.x <= (-ServerParam::PITCH_LENGTH * 0.5 + ServerParam::PENALTY_AREA_LENGTH) && std::fabs(ball_pos.y) <= ServerParam::PENALTY_AREA_WIDTH * 0.5)
        {
            ball_pos.x = -ServerParam::PITCH_LENGTH * 0.5 + ServerParam::PENALTY_AREA_LENGTH + EPS;
            if (ball_pos.y > 0)
            {
                           ...
                           PVector ball_pos)
{
    if (side != RIGHT)
    {
        if (ball_pos.x <= (-ServerParam::PITCH_LENGTH * 0.5 + ServerParam::GOAL_AREA_LENGTH) && std::fabs(ball_pos.y) <= ServerParam::GOAL_AREA_WIDTH * 0.5)
        {
            ball_pos.x = -ServerParam::PITCH_LENGTH * 0.5 + ServerParam::GOAL_AREA_LENGTH + EPS;
        }
    }
...
Referee::moveIntoPenalty(Side side, PVector pos)
{
    if (side != RIGHT)
    {
        if (pos.x > (-ServerParam::PITCH_LENGTH * 0.5 + ServerParam::PENALTY_AREA_LENGTH + ServerParam::instance().ballSize()))
        {
            pos.x = -ServerParam::PITCH_LENGTH * 0.5 + ServerParam::PENALTY_AREA_LENGTH + ServerParam::instance().ballSize();
        }
        ...
        if (std::fabs(pos.y) > (ServerParam::PENALTY_AREA_WIDTH * 0.5 + ServerParam::instance().ballSize()))
        {
            if (pos.y > 0)
            {
                pos.y = ServerParam::PENALTY_AREA_WIDTH * 0.5 + ServerParam::instance().ballSize();
            }
            else
            {
                pos.y = -ServerParam::PENALTY_AREA_WIDTH * 0.5 - ServerParam::instance().ballSize();
...
void Referee::awardFreeKick(const Side side,
                            PVector pos)
{
...
void Referee::awardGoalKick(const Side side,
                            PVector pos)
{
    if (pos.y > 0)
    {
...
void Referee::awardDropBall(PVector pos)
{
    M_stadium.clearBallCatcher();
...
void Referee::awardKickIn(const Side side,
                          PVector pos)
{
    M_stadium.clearBallCatcher();
...
void Referee::awardCornerKick(const Side side,
                              PVector pos)
{
    M_stadium.clearBallCatcher();
    ...
    if (pos.y > 0)
    {
        pos.y = ServerParam::PITCH_WIDTH * 0.5 - ServerParam::instance().cornerKickMargin();
    }
    else
    {
        pos.y = -ServerParam::PITCH_WIDTH * 0.5 + ServerParam::instance().cornerKickMargin();
    ...
    if (side == LEFT)
    {
        pos.x = ServerParam::PITCH_LENGTH * 0.5 - ServerParam::instance().cornerKickMargin();
...
bool Referee::inPenaltyArea(const Side side,
                            const PVector &pos)
{
    if (side != RIGHT)
    {
        ...
        static RArea pen_area(PVector(-ServerParam::PITCH_LENGTH / 2 + ServerParam::PENALTY_AREA_LENGTH / 2.0,
                                      0.0),
                              PVector(ServerParam::PENALTY_AREA_LENGTH + ServerParam::instance().ballSize() * 2,
                                      ServerParam::PENALTY_AREA_WIDTH + ServerParam::instance().ballSize() * 2));
...
bool Referee::isPenaltyShootOut(const PlayMode pm,
                                const Side side)
{
...
bool Referee::crossGoalLine(const Side side,
                            const PVector &prev_ball_pos)
{
    ...
    if (std::fabs(M_stadium.ball().pos().x) <= ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
    {
    ...
    if (std::fabs(prev_ball_pos.x) > ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
    {
    ...
    if (std::fabs(prev_ball_pos.y) > (ServerParam::instance().goalWidth() * 0.5 + ServerParam::instance().goalPostRadius()) && std::fabs(prev_ball_pos.x) > ServerParam::PITCH_LENGTH * 0.5)
    {
    ...
    return std::fabs(y_intercept) <= (ServerParam::instance().goalWidth() * 0.5 + ServerParam::instance().goalPostRadius());
}
...
void TimeRef::analyse()
{
    ...
    if (pm == PM_BeforeKickOff || pm == PM_TimeOver || pm == PM_AfterGoal_Right || pm == PM_AfterGoal_Left || pm == PM_OffSide_Right || pm == PM_OffSide_Left || pm == PM_Illegal_Defense_Left || pm == PM_Illegal_Defense_Right || pm == PM_Foul_Charge_Right || pm == PM_Foul_Charge_Left || pm == PM_Foul_Push_Right || pm == PM_Foul_Push_Left || pm == PM_Back_Pass_Right || pm == PM_Back_Pass_Left || pm == PM_Free_Kick_Fault_Right || pm == PM_Free_Kick_Fault_Left || pm == PM_CatchFault_Right || pm == PM_CatchFault_Left)
    {
        return;
    }
    ...
    if (param.halfTime() > 0)
    {
        ...
        int maximum_time = normal_time + param.extraHalfTime() * param.nrExtraHalfs();

        /* check for penalty shoot-outs, half_time and extra_time. */
        if (M_stadium.time() >= maximum_time)
        {
            if (param.penaltyShootOuts() && M_stadium.teamLeft().point() == M_stadium.teamRight().point())
            {
        ...
        else if (M_stadium.time() >= normal_time)
        {
            ...
            if (!M_stadium.teamLeft().enabled() || !M_stadium.teamRight().enabled())
            {
            ...
            else if (param.goldenGoal() && M_stadium.teamLeft().point() != M_stadium.teamRight().point())
            {
                M_stadium.sendRefereeAudio("time_up");
                M_stadium.changePlayMode(PM_TimeOver);
                return;
            ...
            else if (M_stadium.time() >= (normal_time + (param.extraHalfTime() * extra_count)))
            {
                // when normal halves have just finished (i.e. extra_count==0),
                // referee always check the score difference.
                if (extra_count == 0 && M_stadium.teamLeft().point() != M_stadium.teamRight().point())
                {
                    M_stadium.sendRefereeAudio("time_up");
                    M_stadium.changePlayMode(PM_TimeOver);
                }
...
void Referee::placePlayersInTheirField()
{
    ...
    const bool kick_off_offside(ServerParam::instance().kickOffOffside() && (M_stadium.playmode() == PM_KickOff_Left || M_stadium.playmode() == PM_KickOff_Right));
...
void Referee::clearPlayersFromBall(const Side side)
{
    ...
    const double clear_dist = ((pm == PM_Illegal_Defense_Left || pm == PM_Illegal_Defense_Right || pm == PM_Back_Pass_Left || pm == PM_Back_Pass_Right || ((pm == PM_Foul_Charge_Left || pm == PM_Foul_Push_Left) && inPenaltyArea(LEFT, M_stadium.ball().pos())) || ((pm == PM_Foul_Charge_Right || pm == PM_Foul_Push_Right) && inPenaltyArea(RIGHT, M_stadium.ball().pos())))
                                   ? ServerParam::GOAL_AREA_LENGTH
                                   : ServerParam::KICK_OFF_CLEAR_DISTANCE);
    const bool indirect = (pm == PM_Back_Pass_Left || pm == PM_Back_Pass_Right || pm == PM_Foul_Charge_Left || pm == PM_Foul_Charge_Right || pm == PM_Foul_Push_Left || pm == PM_Foul_Push_Right || pm == PM_IndFreeKick_Left || pm == PM_IndFreeKick_Right);
    ...
    const bool ball_at_corner = (std::fabs(M_stadium.ball().pos().x) > max_x - clear_dist && std::fabs(M_stadium.ball().pos().y) > max_y - clear_dist);
    ...
    for (int loop = 0; loop < 10; ++loop)
    {
        ...
        for (Stadium::PlayerCont::reference p : M_stadium.players())
        {
            ...
            {
                if (indirect && std::fabs(p->pos().x) >= ServerParam::PITCH_LENGTH * 0.5)
                {
                ...
                if (clear_area.inArea(p->pos()))
                {
                    ...
                    if (ball_at_corner && std::fabs(new_pos.x) > ServerParam::PITCH_LENGTH * 0.5 && std::fabs(new_pos.y) > ServerParam::PITCH_WIDTH * 0.5)
                    {
                    ...
                    if (indirect && std::fabs(new_pos.x) > ServerParam::PITCH_LENGTH * 0.5)
                    {
                        double tangent = (new_pos.y - M_stadium.ball().pos().y) / (new_pos.x - M_stadium.ball().pos().x);
                        new_pos.x = ServerParam::PITCH_LENGTH * 0.5 * (new_pos.x > 0.0 ? 1.0 : -1.0);
                        new_pos.y = M_stadium.ball().pos().y + tangent * (new_pos.x - M_stadium.ball().pos().x);
                    }
                    ...
                    if (std::fabs(new_pos.x) > max_x)
                    {
                        ...
                        new_pos.y = (M_stadium.ball().pos().y + (new_pos.y < M_stadium.ball().pos().y
                                                                     ? -tmp_y - 1.0e-5
                                                                     : +tmp_y + 1.0e-5));
                    }
                    ...
                    if (std::fabs(new_pos.y) > max_y)
                    {
                        ...
                        new_pos.x = (M_stadium.ball().pos().x + (new_pos.x < M_stadium.ball().pos().x
                                                                     ? -tmp_x - 1.0e-5
                                                                     : +tmp_x + 1.0e-5));
                        new_pos.y = (new_pos.y < 0.0 ? -max_y : +max_y);
                    }

                    if (std::fabs(new_pos.x) > max_x || std::fabs(new_pos.y) > max_y)
                    {
                        new_pos -= M_stadium.ball().pos();
                        new_pos.rotate(M_PI);
                        new_pos += M_stadium.ball().pos();
...
void Referee::checkFoul(const Player &tackler,
                        const bool foul,
                        bool *detect_charge,
                        bool *detect_yellow,
                        bool *detect_red)
{
    ...
    for (Stadium::PlayerCont::reference p : M_stadium.players())
    {
        ...
        if (player_rel.x < 0.0 || std::fabs(player_rel.y) > p->size() + tackler.size())
        {
...
void BallStuckRef::analyse()
{
    ...
    if (ServerParam::instance().ballStuckArea() <= 0.0 || ServerParam::instance().dropTime() <= 0)
    {
        return;
    }
    ...
    if (M_stadium.ball().pos().distance2(M_last_ball_pos) <= std::pow(ServerParam::instance().ballStuckArea(), 2))
    {
        ++M_counter;
...
void OffsideRef::kickTaken(const Player &kicker,
                           const double accel_r)
{
    setOffsideMark(kicker, accel_r);
}

void OffsideRef::failedKickTaken(const Player &kicker)
{
    checkIntentionalAction(kicker);
}

void OffsideRef::tackleTaken(const Player &tackler,
                             const double accel_r,
                             const bool)
{
...
void OffsideRef::failedTackleTaken(const Player &tackler,
                                   const bool)
{
...
void OffsideRef::checkIntentionalAction(const Player &kicker)
{
    ...
    {
        if (c.player_ == &kicker && c.player_->pos().distance2(M_stadium.ball().pos()) < std::pow(ServerParam::instance().offsideActiveArea(), 2))
        {
...
void OffsideRef::ballTouched(const Player &player)
{
    setOffsideMark(player, 0.0);
}

void OffsideRef::analyse()
{
    if (ServerParam::instance().hfoMode())
    {
        return;
    ...
    if (M_stadium.playmode() == PM_BeforeKickOff || M_stadium.playmode() == PM_KickOff_Left || M_stadium.playmode() == PM_KickOff_Right)
    {
...
void OffsideRef::playModeChange(PlayMode pm)
{
    if (pm != PM_PlayOn)
    {
        M_offside_candidates.clear();
...
void OffsideRef::setOffsideMark(const Player &kicker,
                                const double accel_r)
{
    if (!ServerParam::instance().useOffside())
    {
    ...
    if (M_last_kick_time == M_stadium.time() && M_last_kick_stoppage_time == M_stadium.stoppageTime())
    {
    ...
    if (M_stadium.playmode() == PM_GoalKick_Left || M_stadium.playmode() == PM_GoalKick_Right || M_stadium.playmode() == PM_KickIn_Left || M_stadium.playmode() == PM_KickIn_Right || M_stadium.playmode() == PM_CornerKick_Left || M_stadium.playmode() == PM_CornerKick_Right)
    {
        return;
    }
    ...
    case LEFT:
        ...
        for (Stadium::PlayerCont::const_reference p : M_stadium.players())
        {
            ...
            if (p->side() == LEFT && p->pos().x > offside_line && p->unum() != kicker.unum())
            {
    ...
    case RIGHT:
        ...
        for (Stadium::PlayerCont::const_reference p : M_stadium.players())
        {
            ...
            if (p->side() == RIGHT && p->pos().x < offside_line && p->unum() != kicker.unum())
            {
                M_offside_candidates.push_back(Candidate(p, offside_line, p->pos().y));
            }
        }
...
void OffsideRef::callOffside()
{
    ...
    static const RArea pt(PVector(0.0, 0.0),
                          PVector(ServerParam::PITCH_LENGTH - 2.0 * ServerParam::instance().cornerKickMargin(),
                                  ServerParam::PITCH_WIDTH - 2.0 * ServerParam::instance().cornerKickMargin()));
    static const RArea g_l(PVector(-ServerParam::PITCH_LENGTH / 2.0 + ServerParam::GOAL_AREA_LENGTH / 2.0,
                                   0.0),
                           PVector(ServerParam::GOAL_AREA_LENGTH,
                                   ServerParam::GOAL_AREA_WIDTH));
    static const RArea g_r(PVector(+ServerParam::PITCH_LENGTH / 2.0 - ServerParam::GOAL_AREA_LENGTH / 2.0,
                                   0.0),
                           PVector(ServerParam::GOAL_AREA_LENGTH,
                                   ServerParam::GOAL_AREA_WIDTH));
    ...
    if (M_offside_pos.x > ServerParam::PITCH_LENGTH / 2.0 || g_r.inArea(M_offside_pos))
    {
    ...
    else if (M_offside_pos.x < -ServerParam::PITCH_LENGTH / 2.0 || g_l.inArea(M_offside_pos))
    {
        pos.x = -ServerParam::PITCH_LENGTH / 2.0 + ServerParam::GOAL_AREA_LENGTH;
        pos.y = (M_offside_pos.y > 0 ? 1 : -1) * ServerParam::GOAL_AREA_WIDTH / 2.0;
    }
...
void OffsideRef::checkPlayerAfterOffside()
{
    ...
    for (Stadium::PlayerCont::reference p : M_stadium.players())
    {
        ...
        if (!fld.inArea(p->pos()))
        {
            if (M_stadium.playmode() == PM_OffSide_Right)
            {
                p->moveTo(fld.nearestVEdge(p->pos()) + PVector(ServerParam::instance().offsideKickMargin(), 0));
            }
            else
            {
                p->moveTo(fld.nearestVEdge(p->pos()) - PVector(ServerParam::instance().offsideKickMargin(), 0));
...
void IllegalDefenseRef::kickTaken(const Player &kicker, const double)
{
    ...
    if (M_last_kick_time == M_stadium.time() && (kicker.side() != M_last_kicker_side || M_last_kicker_side == NEUTRAL))
    {
        M_last_kicker_side = NEUTRAL;
        return;
    }
...
void IllegalDefenseRef::analyse()
{
    if (!ServerParam::instance().useIllegalDefense())
    {
        return;
    ...
    if (pm != PM_PlayOn && pm != PM_Illegal_Defense_Left && pm != PM_Illegal_Defense_Right)
    {
        return;
    }
    ...
    for (Stadium::PlayerCont::reference p : M_stadium.players())
    {
        ...
        if (p->side() == LEFT && M_last_kicker_side == RIGHT && M_stadium.ball().pos().x < 0.0 && p->pos().x < left_x && std::fabs(p->pos().y) < half_width)
        {
            left_player_illegal += 1;
            p->addState(ILLEGAL_DEFENSE);
        }
        else if (p->side() == RIGHT && M_last_kicker_side == LEFT && M_stadium.ball().pos().x > 0.0 && p->pos().x > right_x && std::fabs(p->pos().y) < half_width)
        {
...
{
    if (pm == PM_Illegal_Defense_Left || pm == PM_Illegal_Defense_Right)
    {
        M_after_illegal_defense_time = 0;
    }
}
...
void FreeKickRef::kickTaken(const Player &kicker,
                            const double)
{
    if (isPenaltyShootOut(M_stadium.playmode()))
    {
    ...
    {
        if ((M_stadium.playmode() == PM_GoalKick_Left && kicker.side() != LEFT) || (M_stadium.playmode() == PM_GoalKick_Right && kicker.side() != RIGHT))
        {
    ...
    else if (M_stadium.playmode() == PM_OffSide_Left || M_stadium.playmode() == PM_OffSide_Right)
    {
        // do nothing
    }
    ...
    else // PM_PlayOn
    {
        if (M_kick_taken)
        {
            if (M_kick_taker == &kicker && ServerParam::instance().freeKickFaults())
            {
...
void FreeKickRef::tackleTaken(const Player &tackler,
                              const double accel_r,
                              const bool)
{
    kickTaken(tackler, accel_r);
...
void FreeKickRef::ballTouched(const Player &player)
{
    if ((M_stadium.playmode() == PM_GoalKick_Left && player.side() != LEFT) || (M_stadium.playmode() == PM_GoalKick_Right && player.side() != RIGHT))
    {
    ...
    if (M_kick_taken)
    {
        if (M_kick_taker == &player && ServerParam::instance().freeKickFaults())
        {
            if (M_kick_taker->dashCount() > M_kick_taker_dashes)
            {
                M_stadium.setPlayerState(M_kick_taker->side(),
...
void FreeKickRef::analyse()
{
    ...
    if (pm == PM_Back_Pass_Left || pm == PM_Back_Pass_Right || pm == PM_CatchFault_Left || pm == PM_CatchFault_Right)
    {
    ...
    if (goalKick(pm))
    {
        ...
        {
            if (M_kick_taken && ServerParam::instance().properGoalKicks())
            {
                if (ballStopped())
                {
                    if (tooManyGoalKicks())
    ...
    if (pm != PM_PlayOn && pm != PM_BeforeKickOff && pm != PM_TimeOver && pm != PM_AfterGoal_Right && pm != PM_AfterGoal_Left && pm != PM_OffSide_Right && pm != PM_OffSide_Left && pm != PM_Illegal_Defense_Left && pm != PM_Illegal_Defense_Right && pm != PM_Foul_Charge_Right && pm != PM_Foul_Charge_Left && pm != PM_Foul_Push_Right && pm != PM_Foul_Push_Left && pm != PM_Back_Pass_Right && pm != PM_Back_Pass_Left && pm != PM_Free_Kick_Fault_Right && pm != PM_Free_Kick_Fault_Left && pm != PM_CatchFault_Right && pm != PM_CatchFault_Left)
    {
        if (M_stadium.ball().vel().x != 0.0 || M_stadium.ball().vel().y != 0.0)
        {
            M_stadium.changePlayMode(PM_PlayOn);
        }
    }
    ...
    if (pm != PM_PlayOn && pm != PM_TimeOver && pm != PM_GoalKick_Left && pm != PM_GoalKick_Right && pm != PM_OffSide_Left && pm != PM_OffSide_Right && pm != PM_Illegal_Defense_Left && pm != PM_Illegal_Defense_Right && pm != PM_Foul_Charge_Left && pm != PM_Foul_Charge_Right && pm != PM_Foul_Push_Right && pm != PM_Foul_Push_Left && pm != PM_Back_Pass_Left && pm != PM_Back_Pass_Right && pm != PM_Free_Kick_Fault_Left && pm != PM_Free_Kick_Fault_Right && pm != PM_CatchFault_Left && pm != PM_CatchFault_Right)
    {
...
void FreeKickRef::playModeChange(PlayMode pm)
{
    if (pm != PM_PlayOn)
    {
        M_kick_taken = false;
    ...
    if (pm == PM_KickOff_Left || pm == PM_KickIn_Left || pm == PM_FreeKick_Left || pm == PM_IndFreeKick_Left || pm == PM_CornerKick_Left || pm == PM_GoalKick_Left)
    {
        clearPlayersFromBall(RIGHT);
    }
    else if (pm == PM_KickOff_Right || pm == PM_KickIn_Right || pm == PM_FreeKick_Right || pm == PM_IndFreeKick_Right || pm == PM_CornerKick_Right || pm == PM_GoalKick_Right)
    {
        clearPlayersFromBall(LEFT);
    }
...
void FreeKickRef::callFreeKickFault(Side side, PVector pos)
{
    pos = truncateToPitch(pos);
...
bool FreeKickRef::goalKick(PlayMode pm)
{
    return (pm == PM_GoalKick_Right || pm == PM_GoalKick_Left);
}

bool FreeKickRef::freeKick(PlayMode pm)
{
    switch (pm)
    {
    ...
    for (Stadium::PlayerCont::reference p : M_stadium.players())
    {
        ...
        if (p->side() == oppside)
        {
            ...
            if (expand_area.inArea(p->pos()))
            {
                ...
                {
                    new_pos.x = (ServerParam::PITCH_LENGTH / 2 - ServerParam::PENALTY_AREA_LENGTH - size) * oppside;
                }
...
void TouchRef::analyseImpl()
{
    ...
    if (M_stadium.playmode() != PM_AfterGoal_Left && M_stadium.playmode() != PM_AfterGoal_Right && M_stadium.playmode() != PM_TimeOver)
    {
        if (std::fabs(M_stadium.ball().pos().x) > ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
        {
            ...
            if (M_stadium.ball().pos().x > ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
            {
                if (side == RIGHT)
                {
            ...
            else if (M_stadium.ball().pos().x < ServerParam::PITCH_LENGTH * 0.5 - ServerParam::instance().ballSize())
            {
                if (side == LEFT)
                {
        ...
        else if (std::fabs(M_stadium.ball().pos().y) > ServerParam::PITCH_WIDTH * 0.5 + ServerParam::instance().ballSize())
        {
            // check for kick in.
...
void TouchRef::kickTaken(const Player &kicker,
                         const double accel_r)
{
    if (std::fabs(M_stadium.ball().pos().x) <= ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
    {
        if (M_stadium.playmode() == PM_PlayOn && M_last_indirect_kicker && M_last_indirect_kicker != &kicker)
        {
        ...
        if (M_last_touched_time == M_stadium.time() && M_last_touched_accel_r <= accel_r)
        {
...
void TouchRef::tackleTaken(const Player &tackler,
                           const double accel_r,
                           const bool)
{
    kickTaken(tackler, accel_r);
...
void TouchRef::ballTouched(const Player &kicker)
{
    if (std::fabs(M_stadium.ball().pos().x) <= ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize())
    {
        if (M_stadium.playmode() == PM_PlayOn && M_last_indirect_kicker && M_last_indirect_kicker != &kicker)
        ...
        if (M_last_touched_time == M_stadium.time() && M_last_touched_accel_r <= 0.0)
        {
            M_last_touched = &kicker;
...
void TouchRef::playModeChange(PlayMode pm)
{
    if (pm != PM_PlayOn)
    {
...
bool TouchRef::checkGoal()
{
    ...
    if (std::fabs(M_stadium.ball().pos().x) <= ServerParam::PITCH_LENGTH * 0.5 + param.ballSize())
    {
        return false;
    }

    if ((!M_stadium.ballCatcher() || M_stadium.ballCatcher()->side() == LEFT) && crossGoalLine(LEFT, M_prev_ball_pos) && !isPenaltyShootOut(M_stadium.playmode()))
    {
        M_stadium.score(RIGHT);
        announceGoal(M_stadium.teamRight());
        M_after_goal_time = 0;
        M_stadium.placeBall(LEFT, M_stadium.ball().pos());
        if (param.halfTime() >= 0 && param.goldenGoal() && M_stadium.time() >= param.halfTime() * param.nrNormalHalfs())
        {
            M_stadium.changePlayMode(PM_TimeOver);
        }
        else
    ...
    else if ((!M_stadium.ballCatcher() || M_stadium.ballCatcher()->side() == RIGHT) && crossGoalLine(RIGHT, M_prev_ball_pos) && !isPenaltyShootOut(M_stadium.playmode()))
    {
...
void TouchRef::announceGoal(const Team &team)
{
...
bool TouchRef::indirectFreeKick(const PlayMode pm)
{
    switch (pm)
    {
    case PM_IndFreeKick_Right:
...
void CatchRef::kickTaken(const Player &kicker,
                         const double)
{
    if (kicker.side() == LEFT)
    {
...
void CatchRef::tackleTaken(const Player &tackler,
                           const double accel_r,
                           const bool)
{
    kickTaken(tackler, accel_r);
...
void CatchRef::ballTouched(const Player &player)
{
...
void CatchRef::ballCaught(const Player &catcher)
{
    if (ServerParam::instance().hfoMode())
    {
        return;
    ...
    if (M_stadium.playmode() != PM_AfterGoal_Left && M_stadium.playmode() != PM_AfterGoal_Right && M_stadium.playmode() != PM_TimeOver && !inPenaltyArea(catcher.side(), M_stadium.ball().pos()))
    {
    ...
    if (ServerParam::instance().backPasses() && M_stadium.playmode() != PM_AfterGoal_Left && M_stadium.playmode() != PM_AfterGoal_Right && M_stadium.playmode() != PM_TimeOver && M_last_back_passer && M_last_back_passer->team() == catcher.team())
    {
        if (M_last_back_passer == &catcher && M_before_last_back_passer && M_before_last_back_passer->team() != catcher.team())
        {
            // no backpass violation, if last kicker is goalie itself and before kicker is opponent
        }
        else if (M_stadium.time() == M_last_back_passer_time && (!M_before_last_back_passer || M_before_last_back_passer->team() != catcher.team()))
        {
...
void CatchRef::ballPunched(const Player &catcher)
{
    if (isPenaltyShootOut(M_stadium.playmode()))
    {
        return;
    ...
    // check backpass violation
    if (M_stadium.playmode() != PM_AfterGoal_Left && M_stadium.playmode() != PM_AfterGoal_Right && M_stadium.playmode() != PM_TimeOver && M_stadium.time() != M_last_back_passer_time && M_last_back_passer && M_last_back_passer->team() == catcher.team() && ServerParam::instance().backPasses())
    {
        if (M_last_back_passer == &catcher && M_before_last_back_passer && M_before_last_back_passer->team() != catcher.team())
        {
...
void CatchRef::analyse()
{
    ...
    if (M_stadium.ballCatcher() && pm != PM_AfterGoal_Left && pm != PM_AfterGoal_Right && pm != PM_TimeOver && !inPenaltyArea(M_stadium.ballCatcher()->side(), M_stadium.ball().pos()))
    {
...
void CatchRef::playModeChange(PlayMode pmode)
{
    if (pmode != PM_PlayOn)
    {
        M_before_last_back_passer = nullptr;
    ...
    if (pmode == PM_Back_Pass_Left || pmode == PM_Back_Pass_Left)
    {
        M_stadium.clearBallCatcher();
        M_after_back_pass_time = 0;
    }
    else if (pmode == PM_CatchFault_Left || pmode == PM_CatchFault_Left)
    {
        M_stadium.clearBallCatcher();
...
void CatchRef::callBackPass(const Side side)
{
...
void CatchRef::callCatchFault(Side side, PVector pos)
{
    pos = truncateToPitch(pos);
...
void FoulRef::tackleTaken(const Player &tackler,
                          const double,
                          const bool foul)
{
    if (ServerParam::instance().hfoMode())
...
void FoulRef::analyse()
{
    ...
    if (pm == PM_Foul_Charge_Left || pm == PM_Foul_Push_Left)
    {
        ++M_after_foul_time;
    ...
    if (pm == PM_Foul_Charge_Right || pm == PM_Foul_Push_Right)
    {
        ++M_after_foul_time;
...
void FoulRef::callFoul(const Player &tackler)
{
...
void FoulRef::callYellowCard(const Player &tackler)
{
    callFoul(tackler);
...
void FoulRef::callRedCard(const Player &tackler)
{
    callFoul(tackler);
...
void FoulRef::playModeChange(PlayMode pm)
{
    if (pm == PM_Foul_Charge_Left || pm == PM_Foul_Charge_Right || pm == PM_Foul_Push_Left || pm == PM_Foul_Push_Right)
    {
        M_after_foul_time = 0;
    }
}
    ...
    if (M_stadium.playmode() == PM_PlayOn)
    {
        ...
        else
        {
            ...
            for (Stadium::PlayerCont::const_reference p : M_stadium.players())
            {
                ...
                if (ppos.distance2(M_stadium.ball().pos()) < std::pow(ServerParam::instance().kickableArea(), 2))
                {
                    if (p->side() == LEFT)
                    {
                        keeper_poss = true;
...
void KeepawayRef::playModeChange(PlayMode pm)
{
    if (ServerParam::instance().keepAwayMode())
    {
...
bool KeepawayRef::ballInKeepawayArea()
{
    PVector ball_pos = M_stadium.ball().pos();
    return (std::fabs(ball_pos.x) < ServerParam::instance().keepAwayLength() * 0.5 && std::fabs(ball_pos.y) < ServerParam::instance().keepAwayWidth() * 0.5);
}

void KeepawayRef::logHeader()
{
...
void HFORef::playModeChange(PlayMode pm)
{
    if (ServerParam::instance().hfoMode())
    {
        if (pm == PM_PlayOn)
...
bool HFORef::inHFOArea(const PVector &pos)
{
    return (pos.x >= 0 &&
...
void PenaltyRef::startPenaltyShootout()
{
    ...
    if (first_time && param.penaltyShootOuts() && M_stadium.playmode() != PM_BeforeKickOff && M_stadium.teamLeft().point() == M_stadium.teamRight().point() && ((param.halfTime() < 0 && param.nrNormalHalfs() + param.nrExtraHalfs() == 0) || (param.halfTime() >= 0 && (M_stadium.time() >= (param.halfTime() * param.nrNormalHalfs() + param.extraHalfTime() * param.nrExtraHalfs())))))
    {
        ...
        for (Stadium::PlayerCont::reference p : M_stadium.players())
        {
            if (p->isEnabled() && p->side() == side && p->isGoalie())
            {
                p->moveTo(PVector(-M_pen_side * (ServerParam::PITCH_LENGTH / 2 - 10),
                                  10));
            }
        }
...
void PenaltyRef::analyseImpl()
{
    ...
    if (ServerParam::instance().penCoachMovesPlayers() && (pm == PM_PenaltyReady_Left || pm == PM_PenaltyReady_Right || pm == PM_PenaltyTaken_Left || pm == PM_PenaltyTaken_Right))
    {
        if (!bCheckLeft)
        {
...
void PenaltyRef::handleTimeout(bool left_move_check,
                               bool right_move_check)
{
    ...
    if (ServerParam::instance().penCoachMovesPlayers() && (pm == PM_PenaltySetup_Left || pm == PM_PenaltySetup_Right))
    {
        if (!left_move_check)
        {
            penalty_place_all_players(LEFT);
    ...
    if (pm == PM_PenaltyMiss_Left || pm == PM_PenaltyScore_Left || pm == PM_PenaltyMiss_Right || pm == PM_PenaltyScore_Right)
    {
        penalty_init();
    }
    else if (left_move_check && right_move_check)
    {
...
void PenaltyRef::handleTimer(const bool left_move_check,
                             const bool right_move_check)
{
    ...
    if (pm == PM_PenaltyScore_Left || pm == PM_PenaltyScore_Right || pm == PM_PenaltyMiss_Left || pm == PM_PenaltyMiss_Right)
    {
        // freeze the ball
        //         M_stadium.ball().moveTo( M_stadium.ball().pos(),
        //                                 //0.0,
    ...
    if (left_move_check && right_move_check)
    {
        ...
        else if (std::fabs(M_stadium.ball().pos().x) > ServerParam::PITCH_LENGTH * 0.5 + ServerParam::instance().ballSize() || std::fabs(M_stadium.ball().pos().y) > ServerParam::PITCH_WIDTH * 0.5 + ServerParam::instance().ballSize())
        {
            if (M_bDebug)
    ...
    else if (pm == PM_PenaltyReady_Left || pm == PM_PenaltyReady_Right || pm == PM_PenaltyTaken_Left || pm == PM_PenaltyTaken_Right)
    {
...
void PenaltyRef::ballCaught(const Player &catcher)
{
    if (ServerParam::instance().hfoMode())
    {
        return;
    ...
    if (M_stadium.playmode() == PM_PenaltyTaken_Left || M_stadium.playmode() == PM_PenaltyTaken_Right)
    {
    ...
    else if (M_stadium.playmode() == PM_PenaltyReady_Left || M_stadium.playmode() == PM_PenaltyReady_Right)
    {
...
void PenaltyRef::ballPunched(const Player &catcher)
{
    ballCaught(catcher);
}

void PenaltyRef::kickTaken(const Player &kicker,
                           const double)
{
    if (!isPenaltyShootOut(M_stadium.playmode()))
    {
    ...
    else if (M_stadium.playmode() == PM_PenaltySetup_Left || M_stadium.playmode() == PM_PenaltySetup_Right)
    {
        penalty_foul(kicker.side());
    }
    // cannot kick second time after penalty was taken
    else if (!ServerParam::instance().penAllowMultiKicks() && (M_stadium.playmode() == PM_PenaltyTaken_Left || M_stadium.playmode() == PM_PenaltyTaken_Right) && kicker.side() == M_cur_pen_taker)
    {
        penalty_foul(M_cur_pen_taker);
    }
    else if (M_stadium.playmode() == PM_PenaltyReady_Left || M_stadium.playmode() == PM_PenaltyTaken_Left || M_stadium.playmode() == PM_PenaltyReady_Right || M_stadium.playmode() == PM_PenaltyTaken_Right)
    {
        if ((M_stadium.playmode() == PM_PenaltyReady_Left || M_stadium.playmode() == PM_PenaltyReady_Right) && kicker.side() == M_cur_pen_taker && ((LEFT == M_cur_pen_taker && M_sLeftPenTaken.find(kicker.unum()) != M_sLeftPenTaken.end()) || (RIGHT == M_cur_pen_taker && M_sRightPenTaken.find(kicker.unum()) != M_sRightPenTaken.end())))
        {
            // this kicker has already taken the kick
            penalty_foul(M_cur_pen_taker);
        }
        else if (M_last_taker && M_last_taker->side() == M_cur_pen_taker && M_last_taker != &kicker)
        {
            // not a taker player in the same team must not kick the ball.
            penalty_foul(M_cur_pen_taker);
        }
        else if (kicker.side() != M_cur_pen_taker && !kicker.isGoalie())
        {
    ...
    else if (M_stadium.playmode() != PM_PenaltyTaken_Left && M_stadium.playmode() != PM_PenaltyTaken_Right)
    {
        M_stadium.placeBall(M_pen_side, M_stadium.ball().pos());
    }
}

void PenaltyRef::tackleTaken(const Player &tackler,
                             const double accel_r,
                             const bool foul)
{
    if (!isPenaltyShootOut(M_stadium.playmode()))
    ...
    if (detect_charge || detect_yellow || detect_red)
    {
...
void PenaltyRef::playModeChange(PlayMode pm)
{
...
void PenaltyRef::penalty_init()
{
    ...
    M_stadium.placeBall(pm,
                        NEUTRAL,
                        PVector(-M_pen_side * (ServerParam::PITCH_LENGTH / 2 - ServerParam::instance().penDistX()),
                                0.0));
}
...
void PenaltyRef::penalty_foul(const Side side)
{
...
{
    if (M_bDebug)
        std::cerr << "penalty_check_score" << std::endl;
    ...
    {
        if (M_pen_nr_taken % 2 == 0 && (M_stadium.teamLeft().penaltyPoint() != M_stadium.teamRight().penaltyPoint()))
        {
            std::cerr << "Final score: "
                      << M_stadium.teamLeft().penaltyPoint() << "-"
                      << M_stadium.teamRight().penaltyPoint() << std::endl;
...
bool PenaltyRef::penalty_check_players(const Side side)
{
    ...
    if (side == M_cur_pen_taker)
    {
        ...
        else if (std::fabs(posGoalie.x) < ServerParam::PITCH_LENGTH / 2.0 - 1.5 || std::fabs(posGoalie.y) < ServerParam::PENALTY_AREA_WIDTH / 2.0 - 1.5)
        {
            if (M_bDebug)
        ...
        {
            if (outside_player && outside_player->pos().distance(M_stadium.ball().pos()) > 2.0)
            {
                if (M_bDebug)
    ...
    else // other team
    {
        ...
        if (M_stadium.playmode() != PM_PenaltyTaken_Left && M_stadium.playmode() != PM_PenaltyTaken_Right)
        {
            if (std::fabs(posGoalie.x) < ServerParam::PITCH_LENGTH / 2.0 - ServerParam::instance().penMaxGoalieDistX() || std::fabs(posGoalie.y) > ServerParam::instance().goalWidth() * 0.5)
            {
                if (M_bDebug)
    ...
    if (bCheck && outside_player)
    {
        ...
        if ((side == LEFT && M_stadium.playmode() == PM_PenaltySetup_Left && M_sLeftPenTaken.find(outside_player->unum()) != M_sLeftPenTaken.end()) || (side == RIGHT && M_stadium.playmode() == PM_PenaltySetup_Right && M_sRightPenTaken.find(outside_player->unum()) != M_sRightPenTaken.end()))
        {
            if (M_bDebug)
...
void PenaltyRef::penalty_place_all_players(const Side side)
{
    if (side == M_cur_pen_taker)
    {
...
void PenaltyRef::placeTakerTeamPlayers()
{
    const bool bPenTaken = (M_stadium.playmode() == PM_PenaltyTaken_Right || M_stadium.playmode() == PM_PenaltyTaken_Left);
    ...
    for (Stadium::PlayerCont::reference p : M_stadium.players())
    {
        ...
        if (p->side() != M_cur_pen_taker)
            continue;

        if (p == taker)
        {
            if (!bPenTaken && taker->pos().distance(M_stadium.ball().pos()) > 2.0)
            {
...
void PenaltyRef::placeOtherTeamPlayers()
{
    const bool bPenTaken = (M_stadium.playmode() == PM_PenaltyTaken_Right || M_stadium.playmode() == PM_PenaltyTaken_Left);
    const double goalie_line = (M_pen_side == LEFT
...
PenaltyRef::getCandidateTaker()
{
    ...
    for (Stadium::PlayerCont::const_reference p : M_stadium.players())
    {
        ...
        if (sPenTaken.find(p->unum()) != sPenTaken.end())
        {
