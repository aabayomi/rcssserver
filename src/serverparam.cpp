...
bool ServerParam::init(const int &argc,
...
void ServerParam::convertOldConf(const std::string &new_conf)
{
#ifndef RCSS_WIN
    if (std::filesystem::exists(tildeExpand(OLD_SERVER_CONF)) && !std::filesystem::exists(tildeExpand(new_conf)) && std::system("which awk > /dev/null 2>&1") == 0)
    {
...
void ServerParam::setSlowDownFactor()
{
...
template <typename P>
void ServerParam::addParam(const std::string &name,
                           P &param,
...
void ServerParam::setCTLRadius(double value)
{
...
void ServerParam::setKickMargin(double value)
{
...
void ServerParam::setBallSize(double value)
{
...
void ServerParam::setPlayerSize(double value)
{
...
void ServerParam::setNrNormalHalfs(int value)
{
    M_nr_normal_halfs = value < 0 ? 0 : value;
}

void ServerParam::setNrExtraHalfs(int value)
{
    M_nr_extra_halfs = value < 0 ? 0 : value;
}

int ServerParam::getRawSimStep() const
{
    return M_simulator_step / M_slow_down_factor;
}

int ServerParam::getRawSenseBodyStep() const
{
    return M_sense_body_step / M_slow_down_factor;
}

int ServerParam::getRawCoachVisualStep() const
{
    return M_send_vi_step / M_slow_down_factor;
}

int ServerParam::getRawSendStep() const
{
    return M_send_step / M_slow_down_factor;
}

int ServerParam::getRawSynchOffset() const
{
    return M_synch_offset / M_slow_down_factor;
}

void ServerParam::setSynchMode(bool value)
{
...
void ServerParam::setTeamLeftStart(std::string start)
{
    std::replace(start.begin(), start.end(), '\n', ' ');
    M_team_l_start = tildeExpand(start);
}

void ServerParam::setTeamRightStart(std::string start)
{
    std::replace(start.begin(), start.end(), '\n', ' ');
    M_team_r_start = tildeExpand(start);
}

void ServerParam::setTextLogDir(std::string str)
{
    M_text_log_dir = tildeExpand(str);
}

void ServerParam::setGameLogDir(std::string str)
{
    M_game_log_dir = tildeExpand(str);
}

void ServerParam::setKAwayLogDir(std::string str)
{
...
void ServerParam::setCoachMsgFile(std::string str)
{
    M_coach_msg_file = tildeExpand(str);
}

void ServerParam::setFoulDetectProbability(double value)
{
    M_foul_detect_probability = std::max(0.0, std::min(value, 1.0));
}

void ServerParam::setRedCardProbability(double value)
{
    M_red_card_probability = std::max(0.0, std::min(value, 1.0));
}

void ServerParam::setFixedTeamNameLeft(std::string name)
{
    name = check_teamname_format(name);

    if (!M_fixed_teamname_r.empty() && name == M_fixed_teamname_r)
    {
...
void ServerParam::setFixedTeamNameRight(std::string name)
{
    name = check_teamname_format(name);

    if (!M_fixed_teamname_l.empty() && name == M_fixed_teamname_l)
    {
...
bool ServerParam::getInt(const std::string &param,
                         int &value) const
{
    return M_builder->get(param, value);
}

bool ServerParam::getBool(const std::string &param,
                          bool &value) const
{
    return M_builder->get(param, value);
}

bool ServerParam::getDouble(const std::string &param,
                            double &value) const
{
    return M_builder->get(param, value);
}

bool ServerParam::getStr(const std::string &param,
